...
Gate 1 source-only public publication is complete at <https://github.com/redclawanon-rgb/meshcore-bitchat-bridge>. Active Gate 2C target is Eric's three RAKwireless WisMesh Pocket units, documented in `docs/wismesh-pocket-gate2c.md`; use one Pocket first. Eric's Windows home desktop sees the first Pocket as `COM5` / `USB Serial Device (COM5)` / `USB\\VID_239A&PID_8029`, matching RAK4631/nRF52840. Do not use `COM3` (Intel AMT SOL); `COM4` is CH340 and likely unrelated. Tailscale SSH to `station1@home-pc-atl2` works. Python 3.11 is installed at `C:\\Users\\station1\\AppData\\Local\\Programs\\Python\\Python311\\python.exe`; repo clone is at `C:\\Users\\station1\\meshcore-bitchat-bridge`; dry-run/no-open against `COM5` passed. Read-only COM5 probe at 115200 read Meshtastic-style debug output, so MeshCore companion write smoke is blocked until Eric explicitly approves flashing one Pocket to MeshCore RAK4631 USB Companion firmware, or a Meshtastic adapter path is intentionally scoped.

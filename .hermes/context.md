...
Latest verified result: Gate 2I Windows scheduled-task wrapper is installed and live on `home-pc-atl2`. Local repo tests passed with `python3 -m unittest discover -s tests -v` (101 tests after the latest polling fix and third-port wrapper updates). Desktop Python dependencies were verified/installed (`cryptography 49.0.0`, `pyserial 3.5`). Dry-run scheduled task completed with `LastTaskResult=0` and wrote `mode=dry-run-no-ports-opened`. Live task was registered with `-EnableRealPorts -StartNow`; task State=4/Running, `LastTaskResult=267009` (running). A 60-minute passive watch from `2026-06-14T22:46:40Z` to `2026-06-14T23:47:06Z` wrote `.hermes/runtime/meshcore-daemon-watch-20260614T224640Z.jsonl`; 12/13 successful samples reported `Running`/`267009`, one sample had transient SSH exit 255, stderr stayed 0, and daemon log tail had zero `port_open_failed`, `serial_read_error`, or `parse_error`. Active scheduled-task smoke passed after commit `974554a` fixed polling past unknown/non-channel queued sync-next responses: `MeshCoreBitchatBridgeSmoke -EnableRealPorts -StartNow -DurationSeconds 60 -InjectText "pocket1:sched-smoke-20260614-2018"` delivered both the previous queued `sched-smoke-20260614-2012` and current `sched-smoke-20260614-2018` COM5→COM8, saved state with `next_message_id=16931`. Third Pocket inventory: `COM10`, serial `559CC86D2A1B111F`, VID:PID `239A:8029`. Eric clarified COM10 is running latest MeshCore Bluetooth firmware and functions via his iPhone, not USB Companion. Persistent daemon was therefore reverted to COM5/COM8 only; latest log `C:\\Users\\station1\\AppData\\Local\\MeshCoreBitchatBridge\\logs\\meshcore-bridge-20260614-203623.jsonl` opened COM5/COM8 with stderr length 0 and task State=Running/267009. Keep COM10 free for iPhone/Bluetooth use unless Eric explicitly asks to reconfigure/flash it.

...
Gate 1 source-only public publication is complete at <https://github.com/redclawanon-rgb/meshcore-bitchat-bridge>. Gate 2C first live serial smoke is complete for two WisMesh Pockets on Eric's Windows home desktop/Tailscale SSH path. Repo clone is at `C:\\Users\\station1\\meshcore-bitchat-bridge`; Python is `C:\\Users\\station1\\AppData\\Local\\Programs\\Python\\Python311\\python.exe`. `pocket-1`: `COM5`/`VID_239A:PID_8029`/serial `BAE292D6B7431B72`, bootloader was `COM6`/`VID_239A:PID_002A`; `pocket-2`: `COM8`/`VID_239A:PID_8029`/serial `99EF9E1DC9D17560`, bootloader was `COM9`/`VID_239A:PID_002A`. Eric approved flashing both to MeshCore `RAK_4631_companion_radio_usb-v1.16.0-07a3ca9.zip` (SHA-256 `51AEE6E567A8F8E7C1FAE21713ACB3E3283E460675E7BE583D9A2212294D83F5`) using `adafruit-nrfutil`; both DFU runs returned `Device programmed` / `FLASH_EXIT=0`. Post-flash read-only samples on both ports read 0 bytes, and real-port MeshCore companion serial writes succeeded on both (`COM5`: `mode=real-port-opened`, `packet_count=1`, `serial_len=62`; `COM8`: `mode=real-port-opened`, `packet_count=1`, `serial_len=59`). Next gate: configure/verify MeshCore settings and actual over-LoRa delivery between the two nodes; do not assume LoRa delivery yet. Third Pocket remains unflashed/unknown until checked.

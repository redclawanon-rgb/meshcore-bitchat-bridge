...
Latest verified result: `python3 -m unittest discover -s tests -v` ran 99 tests in 0.531s and passed after adding Gate 2I Windows scheduled-task wrapper tests. `python3 tools/meshcore_bridge_daemon.py --port pocket1=COM5 --port pocket2=COM8 --event-log /tmp/meshcore-daemon-dryrun.jsonl --state-file /tmp/meshcore-daemon-state.json --duration-seconds 1` produced `mode=dry-run-no-ports-opened`, `event_count=1`, `delivered_count=0`, `parse_error_count=0`, `reconnect_count=0`, and wrote a JSONL `daemon_plan`; no ports were opened. Gate 2I adds `scripts/windows/Start-MeshCoreBridgeDaemon.ps1`, `scripts/windows/Install-MeshCoreBridgeScheduledTask.ps1`, and `docs/windows-daemon-scheduled-task.md`. Desktop task registration/live smoke is still pending because Windows SSH to `home-pc-atl2` reset/timed out during the 2026-06-14 session.

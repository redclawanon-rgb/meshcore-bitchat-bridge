...
Latest verified result: first live relay smoke exposed a relay loop because stock MeshCore channel messages prepend the device name before `[relay]`, so the original loop guard did not match and the daemon relayed channel echoes repeatedly. The daemon was stopped and process PID 82228 killed to halt the loop. Commit `ef9ba68` fixes this by relaying only unprefixed stock *contact* messages; channel messages are now skipped with `relay_stock_text_skipped_non_contact`, and any contact text containing the relay prefix is skipped with `relay_stock_text_skipped_loop_guard`. Local suite passed 107 tests including `test_relay_policy_only_relays_unprefixed_contact_messages`. Windows repo on home-pc-atl2 was reset to `ef9ba68`; focused test passed; fixed relay daemon restarted at log `C:\\Users\\station1\\AppData\\Local\\MeshCoreBitchatBridge\\logs\\meshcore-bridge-20260614-212327.jsonl`, opened COM5/COM8, stderr length 0, State=Running/267009; watchdog check passed and cron f33031865d0a resumed. Relay remains enabled, but now only contact-origin iPhone messages are relayed once; relayed channel echoes should not be re-relayed.

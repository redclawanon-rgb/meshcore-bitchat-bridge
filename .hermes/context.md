...
- `docs/gate5f-android-first-app-integration-plan.md` — Gate 5F Android-first app integration plan; Eric wants Mac/Xcode work saved for last/Mac VPS, so Android disabled/debug adapter is the next app gate
- `docs/android-app-adapter-stub-gate5g.md` — Gate 5G Android disabled/debug adapter stub evidence, branch/commit, local JDK/Android SDK setup, tests, and non-claims
- `docs/android-debug-hook-plan-gate5h.md` — Gate 5H Android debug hook mapping/patch plan; recommends pure mapper + disabled service wrapper before any live BLE/message-path wiring
- `docs/android-mapper-wrapper-gate5i.md` — Gate 5I Android pure mapper + disabled service wrapper seam evidence, branch/commit, tests, and non-claims
- `docs/android-disabled-message-hook-gate5j.md` — Gate 5J Android default-disabled no-op message hook evidence, branch/commit, tests, and non-claims
- `docs/android-outbound-wrapper-gate5k.md` — Gate 5K Android default-disabled outbound wrapper evidence, branch/commit, tests, and non-claims
- `docs/android-debug-apk-build-gate5l.md` — Gate 5L Android debug APK build preflight evidence, artifact sizes, warnings, and non-claims
- `docs/android-runtime-preflight-gate5m.md` — Gate 5M Android runtime/device preflight checklist and approval boundaries before install/run
- `docs/android-runtime-attempt-gate5n.md` — Gate 5N approved runtime attempt results; system deps/AVDs installed but VPS emulator blocked by missing KVM and unstable software-emulated system services
...
Gate 1 source-only public publication is complete at <https://github.com/redclawanon-rgb/meshcore-bitchat-bridge>. Gates 2C/2D/2E are complete for two MeshCore-flashed WisMesh Pockets on Eric's Windows home desktop. Repo clone is at `C:\\Users\\station1\\meshcore-bitchat-bridge`; Python is `C:\\Users\\station1\\AppData\\Local\\Programs\\Python\\Python311\\python.exe`. `pocket-1`: `COM5`/serial `BAE292D6B7431B72`; `pocket-2`: `COM8`/serial `99EF9E1DC9D17560`; both are flashed to MeshCore USB Companion. Gate 2H daemon state/log/reconnect hardening is complete and live-smoked. Gate 2I Windows scheduled-task wrapper is installed/running live on `home-pc-atl2` with `COM5` and `COM8` opened, a 60-minute passive live watch passed with no daemon-side errors, watchdog job `f33031865d0a` is active, and active scheduled-task message smoke passed. COM10 is the iPhone/Bluetooth MeshCore node and should stay out of the USB Companion daemon unless Eric explicitly asks to reconfigure it. Gate 5F is complete as a plan: app-side work should proceed Android-first, while Mac/Xcode/iOS build/runtime work is saved for last or a rented Mac VPS. Android local branch `/tmp/bitchat-android` `gate5g-android-adapter-stub` contains commits through Gate 5K `55ad1b5`; full `./gradlew :app:testDebugUnitTest --no-daemon` and `./gradlew :app:assembleDebug --no-daemon` passed. Gate 5N runtime attempt is blocked on this VPS: no `/dev/kvm`, no exposed CPU virtualization flags, API 30/35 software-emulated AVDs failed before install/launch, and no physical Android device was connected. Next gate 5O should use a connected physical Android device or a KVM-capable emulator host.

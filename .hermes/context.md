...
Gate 1 source-only public publication is complete at <https://github.com/redclawanon-rgb/meshcore-bitchat-bridge>. Gate 2C first live serial smoke is complete for `pocket-1`: Eric's Windows home desktop/Tailscale SSH path works, repo clone is at `C:\\Users\\station1\\meshcore-bitchat-bridge`, Python is `C:\\Users\\station1\\AppData\\Local\\Programs\\Python\\Python311\\python.exe`, `pocket-1` serial is `BAE292D6B7431B72`, application port is `COM5`/`VID_239A:PID_8029`, bootloader port was `COM6`/`VID_239A:PID_002A`. Eric approved flashing and the device was flashed from Meshtastic-style debug firmware to MeshCore `RAK_4631_companion_radio_usb-v1.16.0-07a3ca9.zip` (SHA-256 `51AEE6E567A8F8E7C1FAE21713ACB3E3283E460675E7BE583D9A2212294D83F5`) using `adafruit-nrfutil`; DFU returned `Device programmed` / `FLASH_EXIT=0`. Post-flash read-only sample read 0 bytes, and `tools/bridge_serial.py --port COM5 --open-real-port 'wismesh pocket serial smoke'` succeeded with `mode=real-port-opened`, `packet_count=1`, `serial_len=57`. Next gate: configure/verify MeshCore settings and a second node for actual over-LoRa delivery; do not assume LoRa delivery yet.

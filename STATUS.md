...
Gate 1 publication is complete for source-only repo creation/push. Gate 2A hardware inventory preflight is recorded in `docs/hardware-inventory-preflight.md`: no candidate `/dev/ttyUSB*`, `/dev/ttyACM*`, or `/dev/serial/...` device was visible on this host/session, `lsusb` is not installed, no real serial/BLE/hardware access was attempted, no-hardware smoke passed, and 59 tests passed. Gate 2B target setup is recorded in `docs/rak4631-target-setup.md` for the loose RAK19003 + RAK4631 assembly. Gates 2C/2D/2E are complete for two MeshCore-flashed WisMesh Pockets on Eric's Windows home desktop: `pocket-1` is `COM5` / serial `BAE292D6B7431B72`; `pocket-2` is `COM8` / serial `99EF9E1DC9D17560`; the 20-message stability loop delivered 20/20 alternating messages with no duplicates or parse errors. Gate 2F MeshCore-side daemon skeleton, Gate 2G daemon state/log hardening, and Gate 2H live unplug/replug reconnect recovery are complete. `tools/meshcore_bridge_daemon.py` opens named ports only with `--open-real-ports`, emits structured JSON events, appends JSONL logs via `--event-log`, persists/reloads `next_message_id` via `--state-file`, drains queued messages by polling `CMD_SYNC_NEXT_MESSAGE`, and has reconnect-aware per-port open/read/write/poll handling. Gate 2H physically mapped screen ID `D0521521` to `pocket2` / `COM8`; unplug produced one `serial_read_error`, closed only `COM8`, emitted eight `port_open_failed` retries while absent, reopened `COM8` after replug, then a post-reconnect smoke delivered `gate2h post reconnect delivery` to `pocket2` with message ID `64573`, `delivered_count=1`, `parse_error_count=0`, and state advanced to `next_message_id=64574`. Remaining gated choices: longer unattended daemon runtime, Windows service/scheduled-task wrapper, flash/use the third Pocket, BLE exploration, scoped stock bitchat compatibility research, or production/security review last. Tags/releases/public announcements are still not created unless separately requested.

...
Gate 1 publication is complete for source-only repo creation/push. Gates 2C/2D/2E are complete for the two USB Companion WisMesh Pockets (`COM5` and `COM8`), Gates 2F/2G/2H/2I are complete for the MeshCore-side daemon, Windows scheduled-task wrapper, state/log persistence, reconnect behavior, watchdogs, passive stability, and active scheduled-task smoke. COM10 is intentionally the iPhone/Bluetooth MeshCore node and should stay out of the USB Companion daemon unless Eric explicitly changes that target. Gate 4 scoped bitchat adapter research and Gate 4A/4B/4C/4D/4E conformance fixtures are complete. Gates 5A/5B/5C/5D/5E established the app-native adapter seam, pump, insertion map, platform-neutral API, and iOS disabled/debug stub. Gate 5F now chooses the Android-first app path and defers Mac/Xcode work for last. Next executable gate: **Gate 5G Android disabled/debug adapter stub**, starting with Android build-tooling preflight because the current VPS lacks Java/Android SDK tooling. Do not wire BLE, mobile message paths, public posts/releases, production/security claims, or stock compatibility without a separate explicit gate.

...
Gate 1 publication is complete for source-only repo creation/push. Gate 2A hardware inventory preflight is recorded in `docs/hardware-inventory-preflight.md`: no candidate `/dev/ttyUSB*`, `/dev/ttyACM*`, or `/dev/serial/...` device was visible on this host/session, `lsusb` is not installed, no real serial/BLE/hardware access was attempted, no-hardware smoke passed, and 59 tests passed. Gate 2B target setup is recorded in `docs/rak4631-target-setup.md` for the loose RAK19003 + RAK4631 assembly. Gate 2C first live serial smoke is complete against Eric's first RAKwireless WisMesh Pocket (`pocket-1`) on the Windows home desktop: device originally appeared as `COM5` / `VID_239A:PID_8029` with Meshtastic-style debug output, Eric approved replacing it, MeshCore `RAK_4631_companion_radio_usb-v1.16.0-07a3ca9.zip` was flashed via bootloader `COM6` / `VID_239A:PID_002A` using `adafruit-nrfutil`, device returned to `COM5`, post-flash Meshtastic debug chatter disappeared, and `tools/bridge_serial.py --port COM5 --open-real-port 'wismesh pocket serial smoke'` succeeded with `mode=real-port-opened`, `packet_count=1`, `serial_len=57`. Remaining gated choices: configure/verify MeshCore settings and second node for over-LoRa delivery, flash/use additional Pocket units, BLE exploration, scoped stock bitchat compatibility research, or production/security review last. Tags/releases/public announcements are still not created unless separately requested.

## Blockers

- Gate 2C first live serial write is complete for `pocket-1`; over-LoRa delivery is not proven yet and needs a configured second MeshCore node.
- Two remaining WisMesh Pocket units are still presumed Meshtastic/unknown firmware unless separately checked/flashed.

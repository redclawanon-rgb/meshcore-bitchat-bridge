...
Gate 1 publication is complete for source-only repo creation/push. Gate 2A hardware inventory preflight is recorded in `docs/hardware-inventory-preflight.md`: no candidate `/dev/ttyUSB*`, `/dev/ttyACM*`, or `/dev/serial/...` device was visible on this host/session, `lsusb` is not installed, no real serial/BLE/hardware access was attempted, no-hardware smoke passed, and 59 tests passed. Gate 2B target setup is recorded in `docs/rak4631-target-setup.md` for the loose RAK19003 + RAK4631 assembly. Active Gate 2C target is Eric's three RAKwireless WisMesh Pocket units, recorded in `docs/wismesh-pocket-gate2c.md`; one Pocket is visible on Eric's Windows home desktop as `COM5` / `USB Serial Device (COM5)` / `USB\\VID_239A&PID_8029`, matching RAK4631/nRF52840. Over Tailscale SSH, Python 3.11 was installed user-scope on the desktop, the repo was cloned at `C:\\Users\\station1\\meshcore-bitchat-bridge`, and the dry-run/no-open command against `COM5` succeeded with `mode=dry-run-no-port-opened`, `packet_count=1`, `serial_len=57`. Remaining gated choices: approve exact guarded real-port smoke on `COM5`, BLE exploration, scoped stock bitchat compatibility research, or production/security review last. Tags/releases/public announcements are still not created unless separately requested.

...
Gate 1 publication is complete for source-only repo creation/push. Gate 2A hardware inventory preflight is recorded in `docs/hardware-inventory-preflight.md`: no candidate `/dev/ttyUSB*`, `/dev/ttyACM*`, or `/dev/serial/...` device was visible on this host/session, `lsusb` is not installed, no real serial/BLE/hardware access was attempted, no-hardware smoke passed, and 59 tests passed. Gate 2B target setup is recorded in `docs/rak4631-target-setup.md` for the loose RAK19003 + RAK4631 assembly. Gates 2C/2D/2E are complete for two MeshCore-flashed WisMesh Pockets on Eric's Windows home desktop: `pocket-1` is `COM5` / serial `BAE292D6B7431B72`; `pocket-2` is `COM8` / serial `99EF9E1DC9D17560`; the 20-message stability loop delivered 20/20 alternating messages with no duplicates or parse errors. Gate 2F MeshCore-side daemon skeleton is complete: `tools/meshcore_bridge_daemon.py` opens named ports only with `--open-real-ports`, emits structured JSON events, drains queued messages by polling `CMD_SYNC_NEXT_MESSAGE` after `msg_waiting` and `channel_data_recv`, uses a time-derived default `message_id_start` to avoid daemon restart collisions, and live-smoked COM5→COM8 with decoded delivered text `gate2f daemon final smoke`, `delivered_count=1`, `parse_error_count=0`, and clean port close events. Remaining gated choices: make the daemon robust as a long-running service with reconnect/persistent queues, run longer/range/interference tests, flash/use the third Pocket, BLE exploration, scoped stock bitchat compatibility research, or production/security review last. Tags/releases/public announcements are still not created unless separately requested.

## Blockers

- Gate 2F MeshCore-side daemon skeleton is complete and live-smoked; production-grade long-running behavior still needs reconnect handling, persistent queues/state, service wrapper, health logging, and longer unattended runtime tests.

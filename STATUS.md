...
A phone MVP build specification is now defined for the remaining path to a real installable Android physical-phone debug APK: platform scope, runtime architecture, acceptance criteria, packaging, security boundary, and queueing/failure behavior are documented as planning artifacts only, with no APK install/run, BLE/serial access, LoRa transmit, public release, or production-security/interoperability claim.
...
- Phone MVP planning docs added: `docs/phone-platform-scope.md`, `docs/phone-runtime-architecture.md`, `docs/android-phone-mvp-acceptance.md`, `docs/android-packaging-plan.md`, `docs/android-phone-security-boundary.md`, and `docs/bridge-queueing-spec.md`.
- README, MVP handoff index, and Android runtime preflight now link the phone MVP build spec while preserving explicit no-install/no-BLE/no-serial/no-production-claim boundaries.
...
- Gate 5F Android-first app integration plan added at `docs/gate5f-android-first-app-integration-plan.md`: Eric asked to save Mac/Xcode work for last and may rent a Mac VPS later, so the next app-side path is Android-first. Local `/tmp/bitchat-android` exists at upstream main `13585a9`; current VPS lacks Java/Android SDK tooling (`./gradlew --version --no-daemon` fails because `JAVA_HOME`/`java` are missing). Gate 5G should add a default-disabled Kotlin adapter stub only after/alongside build-tooling preflight, with no BLE/message-path wiring unless separately approved.
- Gate 5F bridge-repo verification: `python3 -m unittest discover -s tests -v` ran 107 tests in 0.553s and passed.
- Gate 5G Android disabled/debug adapter stub completed in local Android checkout `/tmp/bitchat-android` on branch `gate5g-android-adapter-stub`, commit `5592c3f`. Added `MeshBridgeDebugAdapterConfiguration.kt`, `MeshBridgePublicTextAdapter.kt`, and `MeshBridgePublicTextAdapterTest.kt`; documented in `docs/android-app-adapter-stub-gate5g.md`; Decision `D022` added. Local Android tooling was installed under `/home/openclaw/.local/android-tooling/jdk-17` and `/home/openclaw/.local/android-sdk`. Verification: targeted Gate 5G test `./gradlew :app:testDebugUnitTest --tests 'com.bitchat.android.bridge.MeshBridgePublicTextAdapterTest' --no-daemon` passed; full `./gradlew :app:testDebugUnitTest --no-daemon` passed; test XML reports 5 Gate 5G tests, 0 failures/errors/skips. The adapter defaults disabled and is not wired to BLE, Android message paths, mobile runtime, MeshCore hardware, app signing, or stock bitchat interoperability.
- Gate 5H Android debug hook mapping/patch plan added at `docs/android-debug-hook-plan-gate5h.md`; Decision `D023` added. Re-inspected current Android `MessageHandler.handleBroadcastMessage(...)` lines 383-426, `BluetoothMeshService.onMessageReceived(...)` lines 389-406, and `BluetoothMeshService.sendMessage(...)` lines 703-723. Recommendation: next code gate should be pure mapper + disabled service wrapper tests before any live BLE/message-path wiring.
- Gate 5I Android pure mapper + disabled service wrapper completed in local Android checkout `/tmp/bitchat-android` on branch `gate5g-android-adapter-stub`, commit `55f2ae9`. Added `MeshBridgePublicTextMapper.kt` and `MeshBridgePublicTextMapperTest.kt`; documented in `docs/android-mapper-wrapper-gate5i.md`; Decision `D024` added. Verification: `./gradlew :app:testDebugUnitTest --tests 'com.bitchat.android.bridge.*' --no-daemon` passed; full `./gradlew :app:testDebugUnitTest --no-daemon` passed; test XML reports Gate 5G adapter tests 5/0/0/0 and Gate 5I mapper tests 4/0/0/0. Bridge repo verification after docs/context updates: `python3 -m unittest discover -s tests -v` ran 107 tests in 0.527s and passed. No BLE, live handler tap, Android service runtime, or app send path was wired.
- Gate 5J Android default-disabled no-op message hook completed in local Android checkout `/tmp/bitchat-android` on branch `gate5g-android-adapter-stub`, commit `eb0a330`. Modified `MessageHandler.kt` to own a default-disabled `DebugMeshBridgePublicTextAdapter`, add test-only injection, and call `MeshBridgePublicTextMapper.mapAcceptedPublicMessage(...).emitVerifiedPublicText(...)` after verified broadcast plain-text construction; documented in `docs/android-disabled-message-hook-gate5j.md`; Decision `D025` added. Verification: targeted hook test passed; full `./gradlew :app:testDebugUnitTest --no-daemon` passed; test XML reports adapter 5/0/0/0, mapper 4/0/0/0, hook 2/0/0/0. Normal app behavior remains no-op by default; no BLE, Android service send path, runtime setting, device/emulator, app signing, upstream push, or stock compatibility claim was added.
- Gate 5K Android default-disabled outbound wrapper completed in local Android checkout `/tmp/bitchat-android` on branch `gate5g-android-adapter-stub`, commit `55ad1b5`. Added `MeshBridgeOutboundPublicTextWrapper.kt` and `MeshBridgeOutboundPublicTextWrapperTest.kt`; documented in `docs/android-outbound-wrapper-gate5k.md`; Decision `D026` added. Verification: targeted wrapper test passed; full `./gradlew :app:testDebugUnitTest --no-daemon` passed; test XML reports outbound wrapper 4/0/0/0, adapter 5/0/0/0, mapper 4/0/0/0, hook 2/0/0/0. The wrapper defaults disabled, validates input before any callback, requires an explicitly injected app sender, and does not call `BluetoothMeshService.sendMessage(...)`, BLE, Android lifecycle/runtime, MeshCore hardware, APK signing, upstream push, or stock compatibility paths.
- Gate 5L Android debug APK build preflight completed in local Android checkout `/tmp/bitchat-android` on branch `gate5g-android-adapter-stub` at commit `55ad1b5`; documented in `docs/android-debug-apk-build-gate5l.md`; Decision `D027` added. Verification: `./gradlew :app:assembleDebug --no-daemon` passed in 2m37s and generated local debug APKs for arm64-v8a, armeabi-v7a, universal, x86, and x86_64. Android git status remained clean. No APK install/run, device/emulator, BLE runtime, release signing, upstream push, public distribution, or stock compatibility claim was made.
- Gate 5M Android runtime/device preflight checklist completed at `docs/android-runtime-preflight-gate5m.md`; Decision `D028` added. It defines emulator/no-radio, physical install/launch, and physical BLE-smoke options plus adb/logcat/cleanup commands, redaction notes, and hard stops. No install/run/device/emulator/BLE action was taken; Gate 5N now requires Eric to name the runtime target/scope before any APK install or launch.
- Gate 5N approved Android runtime attempt documented at `docs/android-runtime-attempt-gate5n.md`; Decision `D029` added. Eric approved emulator/no-radio, physical install/launch, and physical BLE-smoke scopes. Sudo system deps were installed successfully and AVDs `gate5n_api35` and `gate5n_api30` were created. No physical device was connected. Emulator runtime remained unusable because `/dev/kvm` is missing and CPU virtualization flags are not exposed; API 35/API 30 software-emulated boots failed before APK install/launch with PackageInstaller/StorageManager null-pointer failures and `DeadSystemException`. No APK install, app launch, BLE action, bridge relay, or stock compatibility claim occurred. Next runtime target should be a connected physical Android device or a KVM-capable emulator host.
...
# Eric sent "3rd from 3" after relay loop fix. Live log C:\Users\station1\AppData\Local\MeshCoreBitchatBridge\logs\meshcore-bridge-20260614-212327.jsonl shows expected no-loop behavior: pocket2/COM8 contact meshcore_text_message "3rd from 3" -> relay_stock_text_sent to pocket1/COM5 -> channel echo "C8BB12CB: [relay] 3rd from 3" -> relay_stock_text_skipped_non_contact; then pocket1/COM5 contact meshcore_text_message "3rd from 3" -> relay_stock_text_sent to pocket2/COM8 -> channel echo "D0521521: [relay] 3rd from 3" -> relay_stock_text_skipped_non_contact. Counts: meshcore_text_message=4, relay_stock_text_sent=2, relay_stock_text_skipped_non_contact=2, stderr empty, task State=Running/267009. No relay bounce loop observed.
# Overnight run: Eric said he is going to bed and approved running as long as needed. Daemon left running. Existing watchdog f33031865d0a remains active every 15m. Added script-only cron d9cb0bdc0877 every 5m for 120 runs using ~/.hermes/scripts/meshcore_relay_overnight_guard.py; it is quiet when healthy but stops the Windows task/process and alerts if relay loop signatures or excessive new relay sends appear. Added one-shot cron 90d8a7e35d4f for 2026-06-15 07:30 America/New_York to summarize overnight stability back to Eric.
```
...
Gate 1 publication is complete for source-only repo creation/push. Gates 2C/2D/2E are complete for the two USB Companion WisMesh Pockets (`COM5` and `COM8`), Gates 2F/2G/2H/2I are complete for the MeshCore-side daemon, Windows scheduled-task wrapper, state/log persistence, reconnect behavior, watchdogs, passive stability, and active scheduled-task smoke. COM10 is intentionally the iPhone/Bluetooth MeshCore node and should stay out of the USB Companion daemon unless Eric explicitly changes that target. Gate 4 scoped bitchat adapter research and Gate 4A/4B/4C/4D/4E conformance fixtures are complete. Gates 5A/5B/5C/5D/5E established the app-native adapter seam, pump, insertion map, platform-neutral API, and iOS disabled/debug stub. Gate 5F chose the Android-first app path and defers Mac/Xcode work for last. Gates 5G/5H/5I/5J/5K completed the local Android disabled adapter, hook plan, pure mapper/wrapper seam, default-disabled inbound hook, and default-disabled outbound wrapper with compile/test verification. Gate 5L completed local debug APK packaging preflight. Gate 5M completed Android runtime/device preflight planning. Gate 5N attempted approved emulator/physical/BLE runtime scopes but this VPS has no KVM/virtualization and no connected physical device, so no APK install/launch occurred. Next executable gate: **Gate 5O physical Android install/launch on a connected device or KVM-capable emulator host**. Do not wire active BLE/mobile sends, public posts/releases, production/security claims, or stock compatibility without a separate explicit gate.

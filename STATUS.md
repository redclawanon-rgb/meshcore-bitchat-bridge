...
Gate 1 publication is complete for source-only repo creation/push. Gate 2A hardware inventory preflight is recorded in `docs/hardware-inventory-preflight.md`: no candidate `/dev/ttyUSB*`, `/dev/ttyACM*`, or `/dev/serial/...` device was visible on this host/session, `lsusb` is not installed, no real serial/BLE/hardware access was attempted, no-hardware smoke passed, and 59 tests passed. Gate 2B target setup is recorded in `docs/rak4631-target-setup.md` for the loose RAK19003 + RAK4631 assembly. Gates 2C/2D/2E are complete for two MeshCore-flashed WisMesh Pockets on Eric's Windows home desktop: `pocket-1` is `COM5` / serial `BAE292D6B7431B72`; `pocket-2` is `COM8` / serial `99EF9E1DC9D17560`; the 20-message stability loop delivered 20/20 alternating messages with no duplicates or parse errors. Gate 2F MeshCore-side daemon skeleton and Gate 2G daemon state/log hardening are complete: `tools/meshcore_bridge_daemon.py` opens named ports only with `--open-real-ports`, emits structured JSON events, appends JSONL logs via `--event-log`, persists/reloads `next_message_id` via `--state-file`, drains queued messages by polling `CMD_SYNC_NEXT_MESSAGE`, and has reconnect-aware per-port open/read/write/poll handling. Live Gate 2G smoke delivered `gate2g hardened daemon smoke`, wrote `16` JSONL lines, saved state `next_message_id=64113`; a second run loaded that state, delivered `gate2g persisted state smoke` with message ID `64113`, appended to `32` JSONL lines, and saved `next_message_id=64114`; both runs had `delivered_count=1`, `parse_error_count=0`, `reconnect_count=0`. Remaining gated choices: live unplug/replug reconnect test, longer unattended daemon runtime, Windows service/scheduled-task wrapper, flash/use the third Pocket, BLE exploration, scoped stock bitchat compatibility research, or production/security review last. Tags/releases/public announcements are still not created unless separately requested.

## Blockers

- Gate 2G daemon state/log hardening is complete and live-smoked; live unplug/replug reconnect recovery, long unattended runtime, Windows service/scheduled-task wrapper, and real bitchat-side adapter are not implemented/proven yet.

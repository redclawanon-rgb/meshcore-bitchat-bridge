...
Gate 1 publication is complete for source-only repo creation/push. Gate 2A hardware inventory preflight is recorded in `docs/hardware-inventory-preflight.md`: no candidate `/dev/ttyUSB*`, `/dev/ttyACM*`, or `/dev/serial/...` device was visible on this host/session, `lsusb` is not installed, no real serial/BLE/hardware access was attempted, no-hardware smoke passed, and 59 tests passed. Gate 2B target setup is recorded in `docs/rak4631-target-setup.md` for the loose RAK19003 + RAK4631 assembly. Gate 2C first live serial smoke is complete for two RAKwireless WisMesh Pockets on Eric's Windows home desktop: `pocket-1` is `COM5` / `VID_239A:PID_8029` / serial `BAE292D6B7431B72`; `pocket-2` is `COM8` / `VID_239A:PID_8029` / serial `99EF9E1DC9D17560`. Both were flashed to MeshCore `RAK_4631_companion_radio_usb-v1.16.0-07a3ca9.zip` using `adafruit-nrfutil` via bootloader ports `COM6` and `COM9` respectively, both returned to application ports, both read-only probes showed no Meshtastic debug chatter, and real-port MeshCore companion serial writes succeeded on both (`COM5` mode `real-port-opened`, `packet_count=1`, `serial_len=62`; `COM8` mode `real-port-opened`, `packet_count=1`, `serial_len=59`). Remaining gated choices: configure/verify MeshCore settings and actual over-LoRa delivery between the two nodes, flash/use the third Pocket, BLE exploration, scoped stock bitchat compatibility research, or production/security review last. Tags/releases/public announcements are still not created unless separately requested.

## Blockers

- Gate 2C first live serial writes are complete for `pocket-1` and `pocket-2`; over-LoRa delivery is not proven yet and needs MeshCore settings/channel verification between the two nodes.
